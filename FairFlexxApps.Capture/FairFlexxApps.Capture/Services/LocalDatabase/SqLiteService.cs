...
namespace FairFlexxApps.Capture.Services.LocalDatabase
{
    public class SqLiteService : ISqLiteService
    {
        #region Properties

        protected const string DatabaseName = "Capture.db3";
        ...
        public AppConfig GetConfig()
        {
            lock (Locker)
            {
                try
                {
                    var appConfig = _database.Get<AppConfig>(a => a.Id == 0);

                    if (appConfig == null)
                    {
                        appConfig = new AppConfig();

                        _database.Insert(appConfig);
                    }
